...
|--------|---------|--------------|
| [`ticket-auto-pipeline`](ticket-auto-pipeline/README.md) | 0.33.0 | **The core.** Takes a Linear ticket and appraises, implements, verifies, and merges it with zero user input. 20+ slash commands, state-machine flow control, deterministic safety gates. |
| [`ticket-planner`](ticket-planner/README.md) | 0.8.23 | **Upstream of the core.** Turns a one-sentence business idea into dependency-ordered, ready-to-execute tickets across 9 planning phases. Supports grill-me validated intent files as optional pre-flight gate. |
| [`fleet-controller`](fleet-controller/README.md) | 0.11.0 | **Above the core.** Dispatches planned tickets, monitors every running pipeline via 14 detection engines, kills and restarts stuck runs, feeds results back to the planner. Bash-only. |
