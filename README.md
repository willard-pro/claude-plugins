...
|--------|---------|--------------|
| [`ticket-auto-pipeline`](ticket-auto-pipeline/README.md) | 0.21.2 | **The core.** Takes a Linear ticket and appraises, implements, verifies, and merges it with zero user input. 20+ slash commands, state-machine flow control, deterministic safety gates. |
...
| [`fleet-controller`](fleet-controller/README.md) | 0.3.2 | **Above the core.** Dispatches planned tickets, monitors every running pipeline via 12 detection engines, kills and restarts stuck runs, feeds results back to the planner. Bash-only. |
