...
test_hb_write_valid_entry_format() {
  ...
  local line; line=$(cat "$tmpfile")
  local fields; fields=$(echo "$line" | awk -F'|' '{print NF}')
  rm -f "$tmpfile"
  [ "$fields" -eq 6 ]
}
...
test_hb_write_rejects_bad_category() {
  ...
  local file_empty=true; [ -s "$tmpfile" ] && file_empty=false
  rm -f "$tmpfile"
...
test_hb_validate_line_valid() {
  ...
  ( source "$LIB_DIR/heartbeat.sh"; hb_validate_line "$valid_line" ) 2>/dev/null
}

test_hb_validate_line_bad_timestamp() {
  local bad_line="2024|api|test-event|ok|msg|{}"
  local exit_code=0
  ( source "$LIB_DIR/heartbeat.sh"; hb_validate_line "$bad_line" ) 2>/dev/null || exit_code=$?
  [ "$exit_code" -ne 0 ]
}
...
test_hb_validate_file_missing_schema_header() {
  ...
  local exit_code=0
  ( source "$LIB_DIR/heartbeat.sh"; hb_validate_file "$tmpfile" ) 2>/dev/null || exit_code=$?
  rm -f "$tmpfile"
  [ "$exit_code" -ne 0 ]
}
