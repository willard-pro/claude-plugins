...
test_step_table_maps_pr_review_to_step_46() {
  ...
  [ -n "$step46" ] || {
    echo "STEP_4_6 not mapped to PR Review"
    return 1
  }
  [ -n "$step5" ] || {
    echo "STEP_5 not mapped to Document + Wiki"
    return 1
  }
}
...
test_pr_review_before_maintenance_in_skill_md() {
  ...
  [ -n "$pr_line" ] || {
    echo "PR Review heading not found"
    return 1
  }
  [ -n "$maint_line" ] || {
    echo "Document + Wiki heading not found"
    return 1
  }
  [ "$pr_line" -lt "$maint_line" ] || {
    echo "PR Review (line $pr_line) not before Document/Wiki (line $maint_line)"
    return 1
  }
}
...
test_verify_transitions_to_pr_review() {
  ...
  grep -q 'NEXT_PHASE=PR-REVIEW' "$skill_md" || {
    echo "No NEXT_PHASE=PR-REVIEW found in SKILL.md"
    return 1
  }

  # Check verify skip heartbeat
  grep -q 'IMPLEMENT → PR-REVIEW (verify skipped)' "$skill_md" || {
    echo "Missing verify skip transition to PR-REVIEW"
    return 1
  }
}
...
test_pr_review_transitions_to_maintenance() {
  ...
  grep -q 'PR-REVIEW → MAINTENANCE' "$skill_md" || {
    echo "Missing PR-REVIEW → MAINTENANCE transition"
    return 1
  }
...
test_maintenance_transitions_to_report() {
  ...
  echo "$maint_section" | grep -q 'NEXT_PHASE=REPORT' || {
    echo "Maintenance section missing NEXT_PHASE=REPORT"
    return 1
  }
}
...
test_detect_resume_step_table() {
  ...
  echo "$maint_done_block" | grep -q 'STEP_6' || {
    echo "MAINTENANCE done not mapped to STEP_6"
    return 1
  }
  ...
  echo "$verify_pass_block" | grep -q 'STEP_4_6' || {
    echo "VERIFY PASS not mapped to STEP_4_6"
    return 1
  }

  # Verify PR-REVIEW open PR no-human-comments → STEP_5
  grep -q 'RESUME_STEP="STEP_5"' "$detect_sh" || {
    echo "No STEP_5 mapping in detect-resume.sh"
    return 1
  }
}
...
test_detect_resume_all_8_steps() {
  ...
  [ -z "$missing" ] || {
    echo "Missing step references in detect-resume.sh:$missing"
    return 1
  }
}
...
test_heartbeat_phase_transition_count() {
  ...
  [ "$count" -ge 8 ] || {
    echo "Only $count phase-transition entries found (need >= 8)"
    return 1
  }
}
...
test_heartbeat_phase_transition_order() {
  ...
  [ "$count" -ge 6 ] || {
    echo "Only $count expected phase transition strings found"
    return 1
  }
}
...
test_skipped_phase_transitions() {
  ...
  grep -q 'APPRAISE → REPRODUCE (skipped)' "$skill_md" || {
    echo "Missing skipped REPRODUCE transition"
    return 1
  }

  # Check for skipped verify transition
  grep -q 'verify skipped' "$skill_md" || {
    echo "Missing skipped VERIFY transition reference"
    return 1
  }
}
...
test_maintenance_agent_uses_distinct_step_name() {
  ...
  grep -q '|MAINTENANCE|wiki-errata|' "$skill_md" || {
    echo "Agent wiki entries not using distinct step name wiki-errata"
    return 1
  }

  # spawn_agent_post still writes MAINTENANCE|maintenance|done (phase bracket)
  grep -q 'PHASE=MAINTENANCE' "$skill_md" || {
    echo "MAINTENANCE phase not found in spawn instructions"
    return 1
  }
}
...
test_spawn_capture_does_not_write_to_pipeline_log() {
  ...
  [ -f "$cap_script" ] || {
    echo "capture-transcript.sh not found"
    return 1
  }
...
test_meta_title_guarded_against_unknown() {
  ...
  grep -B5 'META|title|info|' "$skill_md" | grep -q 'if \[' || {
    echo "Title emission not guarded with if [ condition ]"
    return 1
  }
}
...
test_meta_artifact_guarded_for_absolute_path() {
  ...
  grep -q 'cut -c1.*=.*/' "$skill_md" || {
    echo "Absolute path guard not found for artifact emission"
    return 1
  }
}
